...
void focus(Client *c) {
  if (!c || !ISVISIBLE(c))
    for (c = selmon->stack; c && (!ISVISIBLE(c) || HIDDEN(c)) && c->snext;
         c = c->snext)
      ;
...
void grid(Monitor *m) {
  ...
  if (n == 1) {
    ...
    resize(c, m->mx + (m->mw - cw) / 2 + m->gap->gappx,
           m->my + (m->mh - ch) / 2 + m->gap->gappx, cw - 2 * c->bw,
           ch - 2 * c->bw, 0);
    return;
  }
  if (n == 2) {
    ...
    ch = (m->wh - 2 * m->gap->gappx) * 0.65;
    resize(c, m->mx + m->gap->gappx, m->my + (m->mh - ch) / 2 + m->gap->gappx,
           cw - 2 * c->bw, ch - 2 * c->bw, 0);
    resize(nexttiled(c->next), m->mx + cw + m->gap->gappx + m->gap->gappx,
           m->my + (m->mh - ch) / 2 + m->gap->gappx, cw - 2 * c->bw,
           ch - 2 * c->bw, 0);
    return;
  }
  ...
  overcols = n % cols;
  if (overcols)
    dx = (m->ww - overcols * cw - (overcols - 1) * m->gap->gappx) / 2 -
         m->gap->gappx;
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    ...
    resize(c, cx + m->gap->gappx, cy + m->gap->gappx, cw - 2 * c->bw,
           ch - 2 * c->bw, 0);
  }
}
...
void setgaps(const Arg *arg) {
  Gap *p = selmon->gap = selmon->pertag->gap[selmon->pertag->curtag];
  switch (arg->i) {
  case GAP_TOGGLE:
    p->isgap = 1 - p->isgap;
    break;
